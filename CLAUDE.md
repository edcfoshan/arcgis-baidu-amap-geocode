...
├── config/
│   ├── baidu_keys.txt       # One Baidu API key per line, # for comments
│   ├── amap_keys.txt        # One Amap API key per line
│   ├── amap_poi_categories.json  # Amap POI category code table
│   └── admin_divisions_level.json  # Cached admin division tree
└── core/
...
**Concurrency**: `core_common.calculate_max_workers(speed_level, num_keys, ...)` maps a UI speed choice ("低/中/快/最快") to `ThreadPoolExecutor` worker count via key-count × 30 QPS × multiplier (0.25/0.50/0.75/1.0).
...
- Output is always to FileGDB (`.gdb`) or `in_memory` workspace. The `use_in_memory` boolean parameter toggles this.
